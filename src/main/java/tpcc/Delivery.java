...
public class Delivery {
	...
	public static int delivery(final CassandraConnection conn, final int w_id, final int o_carrier_id) throws Exception {
		new Thread() {
			public void run() {
				try {
					...
					for (d_id = 1; d_id <= 10; d_id++) {
						...
						if (!no_rs.next()) {
							...
							if (_VERBOSE)
								System.out.println(
										String.format("District has no new orders [W_ID=%d, D_ID=%d]", w_id, d_id));
							continue;
						}
						...
						if (!oo_rs.next()) {
							System.out.println(String.format(
									"ERROR_41: Failed to retrieve ORDER record [W_ID=%d, D_ID=%d, O_ID=%d]", w_id, d_id,
									no_o_id));
							//return 41;
						}
						...
						stmt = conn.prepareStatement("SELECT OL_NUMBER, OL_AMOUNT FROM ORDER_LINE "
